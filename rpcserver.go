...
func checkMaxHtlc(maxHtlc uint64, channels []*channeldb.OpenChannel) error {
	var (
		minSupportedHtlc lnwire.MilliSatoshi
		maxSupportedHtlc = lnwire.MilliSatoshi(math.MaxUint64)
	)

	for _, ch := range channels {
		minHtlcMsat := ch.LocalChanCfg.MinHTLC
		if minSupportedHtlc < minHtlcMsat {
			minSupportedHtlc = minHtlcMsat
		}
		...
		// Since the latter is always less than or equal to the former, only
		// check the max in-flight value.
		maxPendingMsats := ch.LocalChanCfg.ChannelConstraints.MaxPendingAmount
		if maxPendingMsats < maxSupportedHtlc {
			maxSupportedHtlc = maxPendingMsats
		}
	}

	if maxSupportedHtlc < lnwire.MilliSatoshi(maxHtlc) {
		return fmt.Errorf("max htlc size of %v msat is too "+
			"large, max supported is %v", maxHtlc, maxSupportedHtlc)
	}

	if minSupportedHtlc > lnwire.MilliSatoshi(maxHtlc) {
		return fmt.Errorf("max htlc size of %v msat is too small, "+
			"min supported is %v", maxHtlc, minSupportedHtlc)
	}
...
func (r *rpcServer) UpdateChannelPolicy(ctx context.Context,
	...
	case *lnrpc.PolicyUpdateRequest_Global:
		...
		targetDbChans = append(openChannels, pendingChannels...)
		if err := checkMaxHtlc(req.MaxHtlcMsat, targetDbChans); err != nil {
			return nil, err
		}
	...
	case *lnrpc.PolicyUpdateRequest_ChanPoint:
		...
		targetChan, err := r.fetchOpenDbChannel(outPoint)
		if err != nil {
			return nil, err
		}
