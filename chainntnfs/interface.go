...
// it missed being notified about, and returns them in a slice.
func GetClientMissedBlocks(chainConn ChainConn, clientBestBlock *BlockEpoch,
	notifierBestHeight int32, chainStoresReorgs bool) ([]BlockEpoch, error) {

	if clientBestBlock == nil {
		return make([]BlockEpoch, 0), nil
	}
	if clientBestBlock.Hash == nil {
		return nil, fmt.Errorf("can't catch up client on blocks: best block " +
			"hash is nil")
	}

	startingHeight := clientBestBlock.Height
	if chainStoresReorgs {
		...
		hashAtBestHeight, err := chainConn.GetBlockHash(int64(clientBestBlock.Height))
		if err != nil {
			return nil, fmt.Errorf("unable to find blockhash for height=%d: %v",
				clientBestBlock.Height, err)
		}

		startingHeight, err = GetCommonBlockAncestorHeight(
			chainConn,
			*clientBestBlock.Hash,
			*hashAtBestHeight,
		)
		if err != nil {
			return nil, fmt.Errorf("unable to find common ancestor: %v", err)
		}
	}
...
// series of blocks, handling a reorg if necessary.
// It returns a slice of the missed blocks.
func HandleMissedBlocks(chainConn ChainConn, txConfNotifier *TxConfNotifier,
	currBestBlock BlockEpoch, newHeight int32,
	chainStoresReorgs bool) (BlockEpoch, []BlockEpoch, error) {
	...
	if chainStoresReorgs {
		...
		hashAtBestHeight, err := chainConn.GetBlockHash(int64(currBestBlock.Height))
		if err != nil {
		...
		if err != nil {
			return currBestBlock, nil, fmt.Errorf("unable to rewind chain: %v", err)
		}
	}
	...
	if err != nil {
		return currBestBlock, nil, fmt.Errorf("unable to get missed blocks: %v", err)
	}
