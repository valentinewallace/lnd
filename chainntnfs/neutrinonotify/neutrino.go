	...
	for {
		...
		case registerMsg := <-n.notificationRegistry:
			...
			case *blockEpochRegistration:
				...
				for _, block := range missedBlocks {
					n.notifyBlockEpochClient(msg.epochID,
		...
		case item := <-n.chainUpdates.ChanOut():
			...
			newBestBlock, err := chainntnfs.RewindChain(
				n.chainConn,
				n.txConfNotifier,
				chainntnfs.BlockEpoch{Height: int32(n.bestHeight), Hash: nil},
				int32(update.height),
			)
			if err != nil {
...
func (n *NeutrinoNotifier) notifyBlockEpochs(newHeight int32, newSha *chainhash.Hash) {
	for clientID, _ := range n.blockEpochClients {
		n.notifyBlockEpochClient(clientID, newHeight, newSha)
	}
}
...
func (n *NeutrinoNotifier) notifyBlockEpochClient(
	clientID uint64, height int32, sha *chainhash.Hash) {
	...
	epochClient, ok := n.blockEpochClients[clientID]
	if !ok {
		return
	}
