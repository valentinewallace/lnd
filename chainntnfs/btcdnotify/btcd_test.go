...
func testCatchUpOnMissedBlocks(miner1 *rpctest.Harness,
	notifier *BtcdNotifier, t *testing.T) {
...
type testCase struct {
	...
	test func(node *rpctest.Harness, notifier *BtcdNotifier, t *testing.T)
}
...
func TestBtcdNotifier(t *testing.T) {
	...
	log.Printf("Running %v BtcdNotifier tests\n", len(ntfnTests))
	var (
	...
	for _, ntfnTest := range ntfnTests {
		...
		success := t.Run(testName, func(t *testing.T) {
			ntfnTest.test(miner, notifier.(*BtcdNotifier), t)
		})
