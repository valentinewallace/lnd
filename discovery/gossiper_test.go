...
var (
	...
	defaultCapacity  = btcutil.Amount(5000000)
)
...
func (r *mockGraphSource) AddEdge(info *channeldb.ChannelEdgeInfo) error {
	...
	// Since the mockGraphSource has no way of fetching the txout, we assign a
	// default value that is also used for the max HTLC when creating channel
	// updates.
	info.Capacity = defaultCapacity
...
func createUpdateAnnouncement(blockHeight uint32, flags lnwire.ChanUpdateFlag,
	...
	a := &lnwire.ChannelUpdate{
		...
		// update validation, we assign it a default value instead of randomly
		// generating it.
		HtlcMaximumMsat: lnwire.NewMSatFromSatoshis(defaultCapacity),
